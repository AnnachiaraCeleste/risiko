 ...
 * @author Annachiara
 */
public class GestionePartitaDaModificare {
    ...
    public GestionePartitaDaModificare() {
    ...
    public GestionePartitaDaModificare(int n_giocatori, TipoPartita tipoPartita) {
        this.n_giocatori = n_giocatori;
        this.tipoPartita = tipoPartita;
        iofGiocatorePartita = new IOObjectFileGiocatorePartita("giocatoriPartita.txt", SEPARATOR);
        iofObiettivoPartita = new IOObjectFileObiettivoPartita("obiettiviPartita.txt", SEPARATOR);
    ...
    public void addTerritoriPartita() throws IOException, TerritorioGiaRegistrato {
        ...
        for (int i = 0, k = 0; i < giocatori.size(); i++) {
            for (int j = 0; k < territoriPartita.size() && j < ((int) 42 / n_giocatori); j++, k++) {
                TerritorioPartita territorio = new TerritorioPartita(territoriPartita.get(idxTerritori[k]).getNome(),
                        territoriPartita.get(idxTerritori[k]).getArma(), giocatori.get(i).getPassword(), 1);
                addTerritorioPartita(territorio);
                giocatori.get(i).setTruppe(giocatori.get(i).getTruppe() - 1);
            }
            if ((n_giocatori == 4 || n_giocatori == 5) && (i == 0 || i == 1 && k < territoriPartita.size())) {
                TerritorioPartita territorio = new TerritorioPartita(territoriPartita.get(idxTerritori[k]).getNome(),
                        territoriPartita.get(idxTerritori[k]).getArma(), giocatori.get(i).getPassword(), 1);
                addTerritorioPartita(territorio);
                giocatori.get(i).setTruppe(giocatori.get(i).getTruppe() - 1);
...
//////////////////////////////FUNZIONI PARTITA//////////////////////////////
     ...
     */
    public int cambiaRinforzi(String psw, String nomeTerritorio, int rinforzi) throws IOException,
        ...
        if (giocatori.get(getLineGiocatore(psw)).getTruppe() < rinforzi) {
        ...
        giocatori.get(getLineGiocatore(psw)).setTruppe(getGiocatore(psw).getTruppe() - rinforzi);
        iofTerritorioPartita.saveData(territori);
        iofGiocatorePartita.saveData(giocatori);
        return getGiocatore(psw).getTruppe();
    }

    /**
     * FASE RINFORZO: metodo che calcola il numero di rinforzi totale che un
     ...
     * @throws CartaNonRegistrataPSWNome
     */
    public void faseRinforzo(String psw) throws IOException, CartaNonRegistrataPSWNome {
        ...
        ArrayList<Giocatore> giocatori = iofGiocatorePartita.loadData();
        giocatori.get(getLineGiocatore(psw)).setTruppe(rinforziTerritori + rinforziContinenti + rinforziCarte);
        iofGiocatorePartita.saveData(giocatori);
    }
     ...
     * @param psw del giocatore
     * @throws IOException
     * @throws exceptions.ControlloObiettivoPerFinePartita
     * @throw FinePartita
     * @throws GiocatoreNonRegistrato
     * @throws TerritorioNonRegistrato
     * @throws ObiettivoNonRegistrato
